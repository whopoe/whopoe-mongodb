...
class Builder extends BaseBuilder {
     ...
     * @var MongoCollection
     */
    protected $collection, $plainCollectionName;
    ...
    /**
     * The column group by date.
     *
     * @var array
     */
    public $groupByDate, $extraColumn;
    ...
    /**
     * Perform mongodb lookup on a query
     *
     * @param $from
     * @param $localField
     * @param $foreignField
     * @return $this
     */
    public function lookup($from, $foreignField, $localField = null)
    {
        $this->lookup = [
            'from' => $from,
            'localField' => $localField == null ? $this->plainCollectionName . '._id' : $this->plainCollectionName . '.' . $localField,
            'foreignField' => $from . '.' . $foreignField,
            'as' => 'lookup'
        ];
    ...
    /**
     * Set the group by date.
     *
     * @param  $column
     * @param  $extraColumn
     * @return $this
     */
    public function groupByDate($column, $extraColumn = null)
    {
        $this->groupByDate = $column;
    ...
    public function getFresh($columns = [])
    {
        ...
        if ($this->groupByDate)
        {
            if ($this->extraColumn)
            {
                $projections = [
                    ...
                    $this->extraColumn => 1,
                    'dateYear' => [
                        '$year' => '$' . $this->groupByDate
                    ],
                    'dateMonth' => [
                        '$month' => '$' . $this->groupByDate
                    ],
                    'dateDay' => [
                        '$dayOfMonth' => '$' . $this->groupByDate
                    ]
                ];
                ...
                $group = [
                    ...
                    'sum' => [
                        '$sum' => '$' . $this->extraColumn
                    ],
                    'avg' => [
                        '$avg' => '$' . $this->extraColumn
                    ]
                ];
            }
            else
            ...
            $pipeline = [];
            $pipeline[] = ['$project' => is_array($this->projections) ? array_merge($projections, $this->projections) : $projections];
            ...
            if ($this->lookup)      $pipeline[] = ['$lookup' => $this->lookup];
        ...
        elseif ($this->lookup or $this->groups or $this->aggregate or $this->paginating)
        {
            $group = [];
    ...
    public function from($collection)
    {
        if ($collection)
        {
            $this->plainCollectionName = $collection;
